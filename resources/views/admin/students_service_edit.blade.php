...
<section id="admin_panel_sizes" name="new-service" class="beautypress-pricing-table-section beautypress-padding-bottom  beautypress-bg" style="background-image: url(img/shop-open-img.jpg)">
	...
	<!-- forma iqneba aq -->
    <center>
	<form method="post" action="{{ url('update_student_data/'.$studentService->id) }}">
  @csrf
  @method('PUT')
  ...
  <div class="mb-3">
    ...
    <input type="text" value="{{ $studentService->service }}" name="service" class="form-control" id="service" >
  </div>
  <div class="mb-3">
    <label for="price" class="form-label">ფასი</label>
    <input type="number" value="{{ $studentService->price }}" name="price" class="form-control" id="price" >
  </div>
